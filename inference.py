...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        return random.choices(population=list(self.keys()), weights=list(self.values()))[0]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        div = int(self.numParticles / len(self.legalPositions))

        for position in self.legalPositions:
            self.particles.extend([position] * div)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        pacmamPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        unique_particles = list(set(self.particles))
        particles_dist = DiscreteDistribution()
        beliefDistribution = self.getBeliefDistribution()

        for up in unique_particles:
            particles_dist[up] = self.getObservationProb(observation, pacmamPosition, up, jailPosition) * \
                                 beliefDistribution[up]

        particles_dist.normalize()

        if (particles_dist.total() == 0):
            self.initializeUniformly(gameState)
            return

        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(particles_dist.sample())
    ...
    def elapseTime(self, gameState):
        ...
        self.particles = []
        for i in range(self.numParticles):
            oldPos = beliefDistribution.sample()
            newPos = newPosDists[oldPos].sample()
            self.particles.append(newPos)
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        dist = DiscreteDistribution()
        unique_values = set(self.particles)

        for uv in unique_values:
            count = self.particles.count(uv)
            dist[uv] = count
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for up in unique_particles:
            up_weight = beliefDistribution[up]

            for i in range(self.numGhosts):
                jailPosition = self.getJailPosition(i)
                obs_prob = self.getObservationProb(observation[i], pacmanPosition, up[i], jailPosition)
                up_weight *= obs_prob

            particles_dist[up] = up_weight
